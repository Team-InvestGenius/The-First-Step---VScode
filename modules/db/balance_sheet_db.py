...
class BalanceSheetDB(DBConnector):

    def select(self, where: str = None):
        """
        BalanceSheet 테이블에서 모든 데이터를 조회하고 반환.
    ...
    def insert(self, data: dict):
        ...
        try:
            with self.db_engine.cursor() as cursor:
                ...
                sql = f"INSERT INTO BalanceSheet ({columns}) VALUES ({values});"
    ...
    def update(self, data: dict, where: str):
        """
        BalanceSheet 테이블의 특정 조건에 맞는 데이터를 업데이트.
        ...
        try:
            with self.db_engine.cursor() as cursor:
                ...
                sql = f"UPDATE BalanceSheet SET {set_clause} WHERE {where};"
                cursor.execute(sql, tuple(data.values()))
                self.db_engine.commit()
        except pymysql.MySQLError as e:
    ...
    def delete(self, where: str):
        """
        BalanceSheet 테이블의 특정 조건에 맞는 데이터를 삭제.
        
        :param where: 삭제 조건 문자열
        """
        try:
            with self.db_engine.cursor() as cursor:
                sql = f"DELETE FROM BalanceSheet WHERE {where};"
                cursor.execute(sql)
                self.db_engine.commit()
        except pymysql.MySQLError as e:
