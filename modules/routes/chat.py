    ...
    try:
        ...
        try:
            ...
            if not room_id:
                ...
                if chatroom_count >= 3:
                    return jsonify({'error': '최대 3개의 채팅방만 생성할 수 있습니다.'}), 400
...
def create_room():
    try:
        ...
        try:
            ...
            # 새로운 채팅방 ID를 확인하기 위해 생성된 채팅방 정보를 다시 조회
            created_room_id = db_connector.get_last_active_chatroom_by_userid(user_id)

            return jsonify({'room_id': created_room_id})
        finally:
            db_connector.close()
    except Exception as e:
        return jsonify({'error': str(e)}), 500
