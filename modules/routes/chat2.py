...
def ask_question():
    try:
        ...
        user_invest_type = data.get("invest_type")
        ...
        if use_gpt:
            ...
            is_portfolio = gpt_response["strategy_request"]

            current_app.logger.info(f"GPT 모델 최종 응답: {model_answer}")
        ...
        if is_portfolio > 0.6:
            ...
            if strategy:
                ...
                strategy_message = (
                    f"**{execute_date}**의 추천 전략을 말씀 드리겠습니다.\n"
                    f"오늘의 추천 전략은 전체 **{n_of_strategies}** 개의 투자 전략을 검토하였고,\n"
                    f"그 결과 고객님의 투자 유형인 **{user_invest_type}**에 적합한 투자 포트폴리오 입니다.\n"
                    f"**{stocks_ratio}** 을 매수하는 것 이며,\n"
                    f"해당 포트폴리오로 투자 하였을 때 예상할 수 있는 연평균 기대수익은 **{annual_return}** 이며,\n"
                    f"최대 낙폭은 **{mdd}** 입니다."
                )
        ...
        response = {
            ...
            "typing": True  # 새로운 메시지임을 나타내는 플래그
        }
